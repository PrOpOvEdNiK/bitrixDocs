(function (exports,main_polyfill_customevent,pull_components_status,ui_vue_components_smiles,im_component_dialog,im_component_textarea,im_component_quotepanel,imopenlines_component_message,imopenlines_component_form,rest_client,main_date,pull_client,im_model,im_controller,im_tools_localstorage,im_provider_rest,im_provider_pull,im_tools_logger,main_md5,im_const,ui_icons,ui_forms,im_utils,ui_vue,ui_vue_vuex) {
	...
	function () {
	  ...
	  function Widget() {
	    ...
	    new ui_vue_vuex.VuexBuilder().addModel(WidgetModel.create().setVariables(widgetVariables)).addModel(im_model.ApplicationModel.create().setVariables(applicationVariables)).addModel(im_model.DialoguesModel.create().useDatabase(cacheDialogues).setVariables({
	      host: this.host
	    })).addModel(im_model.MessagesModel.create().useDatabase(cacheDialogues).setVariables({
	      host: this.host
	    })).addModel(im_model.FilesModel.create().useDatabase(cacheDialogues).setVariables({
	      host: this.host,
	      default: {
	...
	ui_vue.Vue.component('bx-livechat', {
	  ...
	  created: function created() {
	    ...
	    if (!im_utils.Utils.device.isMobile() && !this.widget.common.pageMode) {
	      window.addEventListener('resize', this.getAvailableSpaceFunc = im_utils.Utils.throttle(this.getAvailableSpace, 50));
	    }
	    ...
	    onCreated: function onCreated() {
	      ...
	      if (im_utils.Utils.device.isMobile()) {
	        ...
	        this.$store.dispatch('widget/show').then(function () {
	          _this.widgetCurrentHeight = _this.widgetBaseHeight;
	          _this.widgetCurrentWidth = _this.widgetMinimumWidth;
...
}((this.window = this.window || {}),BX,window,window,window,window,window,window,window,BX,BX,BX,BX.Messenger.Model,BX.Messenger.Controller,BX.Messenger,BX.Messenger.Provider.Pull,BX.Messenger.Provider.Pull,BX.Messenger,BX,BX.Messenger.Const,BX,BX,BX.Messenger,BX,BX));
