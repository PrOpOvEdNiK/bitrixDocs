...
trait TasksHandlers
{
    ...
    public static function onTaskUpdate($taskId, $arData)
    {
        ...
        if (
            ...
            // будем считать, что если пользователь сменил статус задачи с начального - он с ней ознакомился
    ...
    {
        // удаляем уведомление для предыдущего ответственного
        if (!$isNew) {
        ...
        // не ждём ознакомления от самого себя
        ...
        // не добавляем уведомление, если уже лайкали эту задачу
        if (!$isNew) {
        ...
        // добавляем уведомление для текущего ответственного
        ...
        // биндим созданное уведомление к задаче
        ...
        // добавляем обратное уведомление для постановщика
        ...
        // назначаем ответственного за уведомление
        ...
        // добавляем биндинг обратного уведомления к задаче
        $addBinding = NotificationBindingTable::add([
            'NOTIFICATION_ID' => $addWaitNotification->getId(),
            'ENTITY_TYPE' => NotificationBindingTable::TASK_ACKNOWLEDGE_ENTITY,
            'ENTITY_ID' => $taskId,
    ...
    public function OnCommentAdd(int $taskId, array $arData)
    {
        ...
        //очистим уведомления для текущего пользователя
        ...
        // будем считать, что если пользователь что-то комментирует в задаче - он с ней ознакомился
        ...
        // никого не упомянули, больше тут делать нечего
        if (!count($users)) {
            return;
        }

        //очищаем для упомянутых
