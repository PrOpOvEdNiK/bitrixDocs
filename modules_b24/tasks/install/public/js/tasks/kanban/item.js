...
BX.Tasks.Kanban.Item.prototype = {
	...
	render: function()
	{
		...
		{
			var date = new Date();
			var dateParse = data["date_deadline_parse"];
			date.setFullYear(dateParse["YYYY"]);
			date.setMonth(
				parseInt(dateParse["MM"]) - 1,
				parseInt(dateParse["DD"])
			);
			if (dateParse["HH"])
			{
				date.setHours(dateParse["HH"]);
			}
			else if (dateParse["H"])
			{
				date.setHours(dateParse["H"]);
			}
			date.setMinutes(dateParse["MI"]);
			date.setSeconds(0);
			var format = date.getFullYear() ===
						 (new Date()).getFullYear()
						? this.dateFormats["short"][formatLang]
						: this.dateFormats["full"][formatLang];
			data.overdue = Date.now() > date.getTime();
			this.date_deadline.textContent = BX.date.format(format, date);
		}
		else
		{
