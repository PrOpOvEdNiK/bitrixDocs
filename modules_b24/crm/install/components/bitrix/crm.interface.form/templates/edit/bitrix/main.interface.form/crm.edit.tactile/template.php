...
if($arParams['SHOW_SETTINGS'] && $arResult['OPTIONS']['settings_disabled'])
{
	...
	foreach($arResult['TABS'] as $tabID => $tabData)
	{
		...
		{
			$arResult['TABS_META'][$tabID]['fields'][$field['id']] = array('id'=>$field['id'], 'name'=>$field['name'], 'type'=>$field['type']);
		}
	}
}

$undoContainerId = 'undo_container_'.strtolower($arParams['FORM_ID']);
...
$sectionWrapperID = strtolower($arParams['FORM_ID']).'_section_wrapper';
?><div id="<?=$sectionWrapperID?>" class="crm-offer-main-wrap"><?
foreach($arSections as &$arSection):
	$sectionNodePrefix = strtolower($arSection['SECTION_ID']);
	?><table id="<?=$sectionNodePrefix?>_contents" class="crm-offer-info-table"><tbody><?

	$required = (isset($arSection['SECTION_FIELD']['required']) && $arSection['SECTION_FIELD']['required'] === true) ? true : false;
	?><tr id="<?=$arSection['SECTION_ID']?>" draggable="true" data-dragdrop-context="section" data-dragdrop-id="<?=$arSection['SECTION_ID']?>">
		<td colspan="5">
	...
	foreach($arSection['FIELDS'] as &$field):
		...
		?><tr id="<?=strtolower($field["id"])?>_wrap" class="crm-offer-row" draggable="true" data-dragdrop-context="field" data-dragdrop-id="<?=$field["id"]?>"<?=$dragDropType !== '' ? ' data-dragdrop-type="'.$dragDropType.'"' : ''?>>
		...
		elseif($field['type'] === 'lhe'):
			...
			switch($field['type']):
					...
					default:
						...
						break;
				endswitch;
			?></div></td><!-- "crm-offer-info-right" -->
			<td class="crm-offer-info-right-btn"><?
				if(!$required):?>
				<span class="crm-offer-item-del"></span><?
				endif;?><span class="crm-offer-item-edit"></span>
			</td>
		...
		function()
		{
			...
			BX.CrmEditFormManager.create(
				"<?=strtolower($arParams['FORM_ID'])?>",
				{
					...
					hiddenMetaData: window["bxForm_<?=$arParams['FORM_ID']?>"]["oFields"],
					isSettingsApplied: <?=$arResult['OPTIONS']['settings_disabled'] !== 'Y' ? 'true' : 'false'?>,
					canCreateUserField: <?=CCrmAuthorizationHelper::CheckConfigurationUpdatePermission() ? 'true' : 'false'?>,
					...
					userFieldServiceUrl: "<?=SITE_DIR.'bitrix/components/bitrix/crm.config.fields.edit/ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get()?>",
