...
class Order extends Sale\Order
{
	...
	public function addTimelineCheckEntryOnCreate($checkId, array $settings = []) : void
	{
		...
		Crm\Timeline\OrderCheckEntry::create([
			'ENTITY_ID' => (int)$checkId,
			'AUTHOR_ID' => (int)$this->getField('RESPONSIBLE_ID'),
			'SETTINGS' => $settings,
	...
	private function addTimelineEntryOnCreate()
	{
		...
		if ($this->getDealBinding())
		{
			$dealId = $this->getDealBinding()->getDealId();
		}
	 ...
	 * @throws Main\SystemException
	 */
	public static function getInternalEntity()
	{
		return Sale\Internals\OrderTable::getEntity();
	}
}