...
class AppConfiguration
{
	...
	private static function exportFields($step)
	{
		...
		if($entityList[$step])
		{
			...
			if(strpos($entityList[$step], '_') !== false)
			{
	...
	private static function clearFields($option)
	{
		...
		if(isset($entityTypeList[$step]))
		{
			...
			if($clearFull)
			{
				if(strpos($entityTypeList[$step], '_') !== false)
				{
				...
				if(is_array($attribute['CONFIG']))
				{
					...
					if($entityTypeId > 0)
					{
						...
						if(
							...
							&& !empty($import['RATIO']['CRM_STATUS'][$attribute['OPTION']['CATEGORY_ID']])
						)
						{
							$dealCategoryId = $import['RATIO']['CRM_STATUS'][$attribute['OPTION']['CATEGORY_ID']];
	...
	private static function importDetailConfiguration($import)
	{
		...
		elseif(strpos($item['ENTITY'],'DEAL') !== false || strpos($item['ENTITY'],'LEAD') !== false)
		{
			...
			if(static::$entityTypeDetailConfiguration[$entity])
			{
				...
				{
					if(!empty($import['RATIO']['CRM_STATUS'][$id]))
					{
						$id = $import['RATIO']['CRM_STATUS'][$id];
					}
