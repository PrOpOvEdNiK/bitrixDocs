...
function dhtmlXGridObject(a) {
    ...
    this.setColumnSizes    = function (e) {
        for (var a = 0, b = [], c = 0, d = 0; d < this._cCount; d++)if (this.initCellWidth[d] == "*" && !this._hrrar[d])this._awdth = !1, b.push(d); else {
            if (this.cellWidthType == "%") {
                ...
                c > 0.5 && (j++, c--);
                var l = this.cellWidthPX[d] = Math.floor(j), c = c + j - l
    ...
    this.setSizes          = function () {
        if (this.hdr.rows[0]) {
            var e = this.quirks = _isIE && document.compatMode == "BackCompat", a = (this.entBox.offsetWidth - this.entBox.clientWidth) /
                2;
            ...
            if (!this.entBox.offsetWidth &&
                ...
                var d         = !this._wthB && (this.entBox.cmp || this._delta_x) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0, j = !this._wthB && (this.entBox.cmp || this._delta_y) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0, l = this.parentGrid ? !1 : this.objBox.scrollHeight > this.objBox.offsetHeight, m = _isFF ? 18 : 18, k = this.entBox.clientWidth - (this.skin_h_correction || 0) * (e ? 0 : 1) - d, n = this.entBox.clientWidth - (this.skin_h_correction ||
                    0) - d, o = this.entBox.clientHeight - j, p = this.setColumnSizes(n - (l ? m : 0) - (this._correction_x || 0)), r = this.parentGrid ? !1 : this.objBox.scrollWidth > this.objBox.offsetWidth || this.objBox.style.overflowX == "scroll", s = this.hdr.clientHeight, t = this.ftr ? this.ftr.clientHeight : 0, u = k, q = o - s - t;
    ...
    this.startColResize       = function (a) {
        ...
        var i = a.clientX, g = this.hdr.offsetWidth, d = parseInt(b.offsetWidth);
    ...
    this.doColResize          = function (a, b, c, g, d) {
        ...
        this.resized   = b;
        var j          = c + (a.clientX - g), l = d + (a.clientX - g);
    ...
    this.setSortImgState      = function (a, b, c, g) {
        ...
        if (convertStringToBoolean(a)) {
            ...
            if (d) {
                for (var j = 0; j < d.childNodes.length; j++)if (d.childNodes[j]._cellIndexS == b)return this.r_fldSorted =
    ...
    this.setSortImgPos        = function (a, b, c, g) {
        if (!this._hrrar || !this._hrrar[this.r_fldSorted ? this.r_fldSorted._cellIndex : a])if (g || (g = a ? this.hdr.rows[c || 0].cells[a] : this.r_fldSorted), g != null) {
            var d                   = this.getPosition(g, this.hdrBox), j = g.offsetWidth;
        ...
        if (a.button == 2 || _isMacOS && a.ctrlKey) {
            ...
            if (this._ctmndx) {
                ...
                if (this._ctmndx.showContextMenu) {
                    var d = window.document.documentElement, f = window.document.body,
    ...
    this.doClick              = function (a, b, d, g) {
        ...
        if (a.tagName == "TD") {
            ...
            var l = this.row;
            if (d == 1) {
                var m = this.rowsCol._dhx_find(a.parentNode), k = this.rowsCol._dhx_find(this.lastClicked);
                if (m > k)var n = k, o = m; else n = m, o = k;
                for (var p = 0; p < this.rowsCol.length; p++)if (p >=
                    n && p <= o)if (this.rowsCol[p] && !this.rowsCol[p]._sRow)this.rowsCol[p].className.indexOf("rowselected") == -1 && this.callEvent("onBeforeSelect", [this.rowsCol[p].idd, f]) && (this.rowsCol[p].className += " rowselected", this.selectedRows[this.selectedRows.length] = this.rowsCol[p]); else return this.clearSelection(), this.doClick(a, b, 0, g)
            ...
            if (!r && !a.parentNode._sRow)if (this.callEvent("onBeforeSelect", [a.parentNode.idd, f]))if (this.getSelectedRowId() != a.parentNode.idd) {
    ...
    this.moveToVisible        = function (a, b) {
        ...
        try {
            if (a.offsetHeight) {
                var d = a.offsetLeft + a.offsetWidth + 20, f = 0;
            ...
            if (a.offsetHeight)d = a.offsetTop; else var j = this._realfake ? this._fake.rowsAr[a.parentNode.idd] : a.parentNode, d = this.rowsBuffer._dhx_find(j) * this._srdh;
            var l =
                    d + a.offsetHeight + 38;
            if (l > this.objBox.offsetHeight + this.objBox.scrollTop)var m = l - this.objBox.offsetHeight; else d < this.objBox.scrollTop && (m = d - 5);
            if (m)this.objBox.scrollTop = m
        } catch (k) {
        }
    };
    ...
    this.scrollPage           = function (a) {
        if (this.rowsBuffer.length) {
            var b                                         =
                    this._realfake ? this._fake : this, c = Math.floor((b._r_select || this.getRowIndex(this.row.idd) || 0) + a * this.objBox.offsetHeight / (this._srdh || 20));
            c < 0 && (c = 0);
    ...
    this.changeRowId          = function (a, b) {
        if (a != b) {
            ...
            this.rowsAr[a] = null;
            this.rowsAr[b] = c;
            for (var d = 0; d < c.childNodes.length; d++)if (c.childNodes[d]._code)c.childNodes[d]._code =
            ...
            if (this._mat_links && this._mat_links[a]) {
                ...
                for (var j in f)for (d = 0; d < f[j].length; d++)this._compileSCL(f[j][d].original, f[j][d])
            }
    ...
    this.getColLabel          = this.getColumnLabel = function (a, b, c) {
        for (var d = (c || this.hdr).rows[(b || 0) + 1], f = 0; f < d.cells.length; f++)if (d.cells[f]._cellIndexS == a)return _isIE ? d.cells[f].innerText : d.cells[f].textContent;
        return ""
    };
    ...
    this.copyRowContent     = function (a, b) {
        ...
        if (this.isTreeGrid())this._copyTreeGridRowContent(c,
            a, b); else for (var d = 0; d < c.cells.length; d++)this.cells(b, d).setValue(this.cells(a, d).getValue());
    ...
    this.setColumnLabel     = function (a, b, c, d) {
        var f = (d || this.hdr).rows[c || 1], j = f._childIndexes ? f._childIndexes[a] : a;
        if (f.cells[j]) {
    ...
    this.sortField          = function (a, b, c) {
        ...
        if (d && d.tagName == "TH" && this.fldSort.length - 1 >= d._cellIndex && this.fldSort[d._cellIndex] != "na") {
            var f = this.getSortingState(), j = f[0] == a && f[1] == "asc" ? "des" : "asc";
            if (this.callEvent("onBeforeSorting", [a, this.fldSort[a], j])) {
                ...
                this.fldSorted   = d;
                this.r_fldSorted = c;
                var k            = this.hdr.rows[1], k = c.parentNode, n = k._childIndexes ? k._childIndexes[d._cellIndex] : d._cellIndex;
                this.setSortImgPos(!1,
                    !1, !1, c)
            }
        }
    };
    ...
    this.setColTypes        = function (a) {
        ...
        for (var b = 0; b < this.cellType.length; b++) {
            ...
            window["eXcell_" + this.cellType[b]] || dhtmlxError.throwError("Configuration", "Incorrect cell type: " + this.cellType[b],
                [this, this.cellType[b]])
        }
    };
    this.setColSorting      = function (a) {
        this.fldSort = a.split(this.delim);
        for (var b = 0; b < this.fldSort.length; b++)if (this.fldSort[b].length > 4 && typeof window[this.fldSort[b]] == "function") {
            if (!this._customSorts)this._customSorts = [];
    ...
    this.setStyle           = function (a, b, c, d) {
        ...
        for (var f = [
            "#" + this.entBox.id + " table.hdr td", "#" + this.entBox.id + " table.obj td", "#" + this.entBox.id + " table.obj tr.rowselected td.cellselected",
    ...
    this._fixAlterCss       = function (a) {
        ...
        if (this._cssEven || this._cssUnEven)for (var b = a = a || 0, c = a; c < this.rowsCol.length; c++)if (this.rowsCol[c] && this.rowsCol[c].style.display != "none")this.rowsCol[c]._cntr ? b = 1 : (this.rowsCol[c].className = this.rowsCol[c].className.indexOf("rowselected") != -1 ? b % 2 == 1 ? this._cssUnEven + " rowselected " + (this.rowsCol[c]._css || "") : this._cssEven + " rowselected " + (this.rowsCol[c]._css || "") : b % 2 == 1 ? this._cssUnEven + " " + (this.rowsCol[c]._css || "") : this._cssEven + " " + (this.rowsCol[c]._css || ""), b++)
    };
    this.clearChangedState  = function () {
        for (var a = 0; a < this.rowsCol.length; a++) {
            var b = this.rowsCol[a];
            if (b && b.childNodes)for (var c = b.childNodes.length, d = 0; d < c; d++)b.childNodes[d].wasChanged = !1
        }
    };
    this.getChangedRows     = function (a) {
        ...
        this.forEachRow(function (c) {
            ...
            if (d.tagName == "TR") {
                ...
                if (a && d._added)b[b.length] = d.idd; else for (var j = 0; j < f; j++)if (d.childNodes[j].wasChanged) {
                    b[b.length] = d.idd;
                    break
                }
            }
    ...
    this._serialise             = function () {
        ...
        if (this.isTreeGrid())this._h2.forEachChildF(0, function (b) {
            ...
            a.push(b)
        }); else for (var c = 0; c < this.rowsBuffer.length; c++)if (this.rowsBuffer[c] && (!this._chAttr || !this.rowsBuffer[c]._locator)) {
    ...
    this._serializeRow          = function (a, b) {
        ...
        if (d.length)for (b = 0; b < d.length; b++)c.push(" " + d[b] + "='" + a._attrs[d[b]] + "'");
        c.push(">");
        ...
        for (var l = !1, m = 0; m < this._cCount; m++)if (!this._srClmn || this._srClmn[m]) {
            var k = this.cells3(a, m);
            c.push("<cell");
            if (f.length)for (b = 0; b < f.length; b++)c.push(" " + f[b] + "='" + k.cell._attrs[f[b]] + "'");
            zxVal = k[this._agetm]();
            ...
            if (this._ecspn && k.cell.colSpan)for (var n = 0; n < k.cell.colSpan - 1; n++)c.push("<cell/>"), m++
        }
        return this._onlChAttr && !l && !a._added ? "" : c.join("")
    };
    this._serialiseConfig       = function () {
        for (var a = "<head>", b = 0; b < this.hdr.rows[0].cells.length; b++)if (!this._srClmn || this._srClmn[b]) {
            ...
            if (d)for (var f = 0; f < d.keys.length; f++)a += "<option value='" + d.keys[f] + "'>" + d.values[f] + "</option>";
            a += "</column>"
        }
        return a += "</head>"
    };
    this.serialize              = function () {
        ...
        if (this._sUDa && this.UserData.gridglobaluserdata)for (var b = this.UserData.gridglobaluserdata.getKeys(), c = 0; c < b.length; c++)a += "<userdata name='" + b[c] + "'>" + this.UserData.gridglobaluserdata.get(b[c]) + "</userdata>";
...
dhtmlXGridObject.prototype = {
    ...
    setColumnsVisibility:        function (a) {
        if (a)this._ivizcol =
            a.split(this.delim);
        if (this.hdr.rows.length && this._ivizcol)for (var b = 0; b < this._ivizcol.length; b++)this.setColumnHidden(b, this._ivizcol[b])
    },
    _fixHiddenRowsAll:           function (a, b, c, d, e) {
        for (var e = e || "_cellIndex", h = a.rows.length, i = 0; i < h; i++) {
            var g = a.rows[i].childNodes;
            if (g.length != this._cCount)for (var f = 0; f < g.length; f++) {
                if (g[f][e] == b) {
                    g[f].style[c] = d;
                    break
                }
    ...
    destructor:                  function () {
        ...
        if (this.formInputs)for (var a = 0; a < this.formInputs.length; a++)this.parentForm.removeChild(this.formInputs[a]);
        var b;
    ...
    setNumberFormat:             function (a, b, c, d) {
        var e = a.replace(/[^0\,\.]*/g, ""), h = e.indexOf(".");
        h > -1 && (h = e.length - h - 1);
        var i = e.indexOf(",");
    ...
    _aplNF:                      function (a, b) {
        var c = this._maskArr[b];
        if (!c)return a;
        var d   = (parseFloat(a) < 0 ? "-" : "") + c[2], a = Math.abs(Math.round(parseFloat(a) * Math.pow(10, c[0] > 0 ? c[0] : 0))).toString(), a = (a.length < c[0] ? Math.pow(10, c[0] + 1 - a.length).toString().substr(1, c[0] + 1) + a.toString() : a).split("").reverse();
        a[c[0]] = (a[c[0]] || "0") + c[4];
        if (c[1] > 0)for (var e = (c[0] > 0 ? 0 : 1) + c[0] + c[1]; e <
    ...
    _parseHead:                  function (a) {
        ...
        if (b.length) {
            var c = this.xmlLoader.doXPath("./column", b[0]), d = this.xmlLoader.doXPath("./settings", b[0]), e = "setInitWidths", h = !1;
            if (d[0])for (var i =
            ...
            if (c.length > 0) {
                this.hdr.rows.length > 0 && this.clearAll(!0);
                for (var g = [[], [], [], [], [], [], [], [], []], f = ",width,type,align,sort,color,format,hidden,id".split(","), j = [
                    "", e,
                ...
                this.setHeader(g[0]);
                for (l = 0; l < j.length; l++)if (j[l])this[j[l]](g[l].join(this.delim));
                for (l = 0; l < c.length; l++)if (this.cellType[l].indexOf("co") == 0 || this.cellType[l] == "clist") {
                    ...
                    if (k.length) {
                        ...
                        } else for (var o = this.getCombo(l), m = 0; m < k.length; m++)o.put(k[m].getAttribute("value"), k[m].firstChild ? k[m].firstChild.data : "")
                    }
    ...
    setColspan:                  function (a, b, c) {
        if (this._ecspn) {
            ...
            if (d._childIndexes && d.childNodes[d._childIndexes[b]]) {
                var e = d._childIndexes[b], h = d.childNodes[e],
                    i                         = h.colSpan;
                h.colSpan                     = 1;
                if (i && i != 1)for (var g = 1; g < i; g++) {
            ...
            if (c && c > 1) {
                if (d._childIndexes)e = d._childIndexes[b]; else {
                    e               = b;
                    d._childIndexes = [];
    ...
    _onHeaderDblClick:           function (a) {
        var b = this.grid, c = b.getFirstParentOfType(_isIE ? event.srcElement :
                                                      a.target, "TD");
        if (!b._eCAS)return !1;
    ...
    _createHRow:                 function (a, b) {
        ...
        var g = a[1], c = document.createElement("TR");
        b.rows[0].parentNode.appendChild(c);
        for (i = 0; i < a[0].length; i++)if (a[0][i] == "#cspan") {
            ...
            for (var j = b.rows.length - 2, l = !1, f = null; !l;) {
                for (var f = b.rows[j], m = 0; m < f.cells.length; m++)if (f.cells[m]._cellIndex == i) {
                    l = m + 1;
                    break
                }
                j--
            ...
            if (typeof a[0][i] == "object")k.appendChild(a[0][i]); else if (k.innerHTML = this.forceDivInHeader ? "<div class='hdrcell'>" + (a[0][i] || "&nbsp;") + "</div>" : a[0][i] || "&nbsp;", (a[0][i] || "").indexOf("#") != -1)if (d = a[0][i].match(/(^|{)#([^}]+)(}|$)/)) {
    ...
    changeCellType:              function (a, b, c) {
        var c            = c || this.cellType[b], d = this.cells3(a, b), e = d.getValue();
        d.cell._cellType = c;
        d                = this.cells3(a, b);
        d.setValue(e)
    },
    ...
    _refreshFromXML:             function (a) {
        ...
        for (var e = a.doXPath("//row"), h = 0; h < e.length; h++) {
            var i = e[h], g = i.getAttribute("id");
            d[g]  = !1;
            c     = i.parentNode.getAttribute("id") ||
                c;
            if (this.rowsAr[g] && this.rowsAr[g].tagName != "TR")this._h2 ? this._h2.get[g].buff.data = i : this.rowsBuffer[this.getRowIndex(g)].data = i, this.rowsAr[g] = i; else if (this.rowsAr[g])this._process_xml_row(this.rowsAr[g], i, -1), this._postRowProcessing(this.rowsAr[g], !0); else if (this._refresh_mode[1]) {
                var f = {
                    idd:      g,
                    data:     i,
        ...
        if (this._refresh_mode[2])for (g in d)d[g] && this.rowsAr[g] && this.deleteRow(g);
    ...
    _key_events:                 {
        k13_1_0:     function () {
            ...
            if (a)this.selectCell(a.parentNode, a._cellIndex, this.row != a.parentNode,
                !1, !0), this._still_active = !0
        }, k9_0_1:   function () {
            this.editStop();
        ...
        k34_0_0:     function () {
            ...
            if (this.editor && this.editor.combo)this.editor.shiftNext(); else {
                ...
                if (this.rowsBuffer[b])a._r_select =
                    null, this.selectCell(b, this.cell._cellIndex, !0), a.pagingOn && a.showRow(a.getRowId(b)); else {
    ...
    _process_xml_row:            function (a, b) {
        ...
        if (this._ud_enabled)for (var e = this.xmlLoader.doXPath("./userdata",
        ...
        for (g = 0; g < c.length; g++) {
            ...
            if (f) {
                var j = a._childIndexes ? a._childIndexes[g] : g, l = f.getAttribute("type");
                if (a.childNodes[j]) {
    ...
    _fillRow:                    function (a, b) {
        ...
        for (var c = 0; c < a.childNodes.length; c++)if (c < b.length || this.defVal[c]) {
            var d = a.childNodes[c]._cellIndex, e = b[d], h = this.cells4(a.childNodes[c]);
            if (this.defVal[d] && (e == "" ||
    ...
    _postRowProcessing:          function (a, b) {
        ...
        for (var c = 0, d = 0; d < a.childNodes.length; d++) {
            var e = a.childNodes[d], h = e._cellIndex, i = e._attrs.style || a._attrs.style;
            i && (e.style.cssText += ";" + i);
    ...
    _process_xml:                function (a) {
        ...
        if (c.tagName == this.xml.top) {
            ...
            this._parseHead(c);
            var e = a.doXPath(this.xml.row, c), h = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("pos") || 0), i = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("total_count") || 0), g = !1;
            ...
            for (var f = 0; f < e.length; f++)if (!this.rowsBuffer[f + h]) {
    ...
    _process_json:               function (a, b) {
        ...
        } else for (c = 0; c < a.rows.length; c++)e = a.rows[c].id, this.rowsBuffer.push({
            idd:      e,
            data:     a.rows[c],
            _parser:  this._process_json_row,
            _locator: this._get_json_data
        }), this.rowsAr[e] = a[c];
        if (a.dhx_security)dhtmlx.security_key =
            a.dhx_security;
        this.render_dataset();
        this._parsing = !1
    },
    render_dataset:              function (a, b) {
        ...
        for (var c = a; c < b; c++) {
            ...
            if (d == -1) {
                if (this.xmlFileUrl && this.callEvent("onDynXLS",
                        [c, this._dpref ? this._dpref : b - c]))this.load(this.xmlFileUrl + getUrlSymbol(this.xmlFileUrl) + "posStart=" + c + "&count=" + (this._dpref ? this._dpref : b - c), this._data_type);
                b = c;
                break
            }
        ...
        if (this._srnd && !this._fillers) {
            ...
            for (this._fillers = []; h > 0;) {
                var i = _isIE ? Math.min(h, 5E4) : h, g = this._add_filler(b, i);
    ...
    _sortCore:                   function (a, b, c, d, e) {
        ...
        if (b == "cus") {
            ...
            var e = parseFloat(d[a.idd]), e = isNaN(e) ? -99999999999999 : e, i = parseFloat(d[b.idd]), i = isNaN(i) ? -99999999999999 : i;
            return c == "asc" ? e - i : i - e
        }); else if (b == "date")e[h](function (a, b) {
            var e = Date.parse(d[a.idd]) || Date.parse("01/01/1900"), i = Date.parse(d[b.idd]) ||
    ...
    _reset_view:                 function (a) {
        if (this.obj.rows[0]) {
            ...
            if (_isKHTML)for (var d = b.parentNode.childNodes.length - 1; d >= 0; d--)b.parentNode.childNodes[d].tagName == "TR" && b.parentNode.removeChild(b.parentNode.childNodes[d], !0); else if (_isIE)for (d =
                                                                                                                                                                                                                      b.childNodes.length - 1; d >= 0; d--)b.childNodes[d].removeNode(!0); else b.innerHTML = "";
            b.appendChild(c);
            this.rowsCol = dhtmlxArray();
            this._sst && this.enableStableSorting(!0);
            this._fillers = this.undefined;
...
(function () {
    ...
    function e(a, b) {
        ...
        for (var d = 1, e = [], h = 0; h < b.length; h++)if (typeof b[h] == "object" && b[h].length)for (var k = 0; k < b[h].length; k++)c(h, k, b[h][k]); else c(h,
            0, b[h]);
        for (h = 0; h < e.length; h++)for (k = 0; k < e[0].length; k++) {
            var n   = e[h][k];
            e[h][k] = (n || "").toString() || "&nbsp;";
            if (n && n.colspan)for (var o = 1; o < n.colspan; o++)c(k + o, h, "#cspan");
            if (n && n.rowspan)for (o = 1; o < n.rowspan; o++)c(k, h + o, "#rspan")
        }
        this.setHeader(e[0]);
    ...
    a._getRowData       = function (a) {
        ...
        for (var d = this.obj.getRowById(a), e = 0; e < this.obj._cCount; e++) {
            var h = this.obj._c_order ? this.obj._c_order[e] :
                    e, i = this.obj.cells(d.idd, e);
            if (!this._changed || i.wasChanged())this._endnm ? c[this.obj.getColumnId(e)] = i.getValue() : c["c" + h] = i.getValue()
        }
        var g = this.obj.UserData[a];
        if (g)for (var f = 0; f < g.keys.length; f++)g.keys[f] && g.keys[f].indexOf("__") != 0 && (c[g.keys[f]] = g.values[f]);
        if (g = this.obj.UserData.gridglobaluserdata)for (f = 0; f < g.keys.length; f++)c[g.keys[f]] = g.values[f];
        return c
    };
