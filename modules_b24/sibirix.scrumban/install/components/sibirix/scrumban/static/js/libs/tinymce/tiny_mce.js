...
(function(tinymce) {
	...
	tinymce.html.Schema = function(settings) {
		...
		blockElementsMap = createLookupTable('block_elements', 'hr table tbody thead tfoot ' +
						'th tr td li ol ul caption dl dt dd noscript menu isindex samp option datalist select optgroup a', textBlockElementsMap);
...
(function(tinymce) {
	...
	tinymce.UndoManager = function(editor) {
		...
		editor.onKeyUp.add(function(editor, e) {
			...
			if ((keyCode >= 32 && keyCode <= 36) || (keyCode >= 37 && keyCode <= 40) || keyCode == 45 || keyCode == 13 || e.ctrlKey) {
				addNonTypingUndoLevel();
			}
		});
		...
		editor.onKeyDown.add(function(editor, e) {
			...
			if ((keyCode >= 32 && keyCode <= 36) || (keyCode >= 37 && keyCode <= 40) || keyCode == 45) {
				if (self.typing) {
					addNonTypingUndoLevel();
				}
	...
	tinymce.EnterKey = function(editor) {
		...
		function handleEnterKey(evt) {
			...
			if (parentBlockName == 'PRE' && settings.br_in_pre !== false) {
				...
				if (parentBlockName != 'A' && ((!newBlockName && !shiftKey && parentBlockName != 'LI') || (newBlockName && shiftKey))) {
					insertBr();
					return;
				}
			}
			...
			if (isCaretAtStartOrEndOfBlock()) {
				...
				if (/^(H[1-6]|PRE|A)$/.test(parentBlockName) && containerBlockName != 'HGROUP') {
