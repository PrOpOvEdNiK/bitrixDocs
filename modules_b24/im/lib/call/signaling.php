...
class Signaling
{
	...
	public function sendAnswer($senderId, $callInstanceId, $isMobile)
	{
		$config = array(
			'call' => $this->call->toArray(),
			'senderId' => $senderId,
	...
	public function sendPing($senderId, $requestId)
	{
		...
		return $this->send('Call::ping', $toUserIds, $config);
	}

	public function sendNegotiationNeeded($senderId, $toUserId, $restart)
	{
	...
	public function sendIceCandidates($senderId, $toUserId, $connectionId, array $iceCandidates)
	{
	...
	protected function send($command, $users, array $params = [], $push = null, $ttl = 5)
	{
		if(!Loader::includeModule('pull'))
			return false;
