;(function (window)
{
	...
	MessengerCommon.prototype.enableScroll = function(element, max, scroll)
	{
		...
		if (lastUnreadMessage)
		{
			...
			if (!scrollResult.top)
			{
				this.scrollToNode(lastUnreadMessage.parentNode.parentNode.parentNode.parentNode.parentNode);
				return false;
			}
		}
		...
		var pullHandler = BX.delegate(function(command,params,extra)
		{
			...
			else if (command == 'message' || command == 'messageChat')
			{
				...
				)
				{
					...
					this.BXIM.messenger.showMessage[this.BXIM.messenger.currentTab].push(params.message.id.toString());
