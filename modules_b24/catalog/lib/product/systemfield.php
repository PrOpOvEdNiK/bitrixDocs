...
final class SystemField
{
	...
	public static function prepareRow(array &$row)
	{
		...
		if (
			isset($fieldList['MARKING_CODE_GROUP'])
			&& array_key_exists('MARKING_CODE_GROUP', $row)
		)
		{
			$value = null;
			if ($row['MARKING_CODE_GROUP'] !== null && self::initHighloadBlock())
			{
				$xmlId = $row['MARKING_CODE_GROUP'];
